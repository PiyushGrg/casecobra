...
import { SaveConfigArgs, saveConfiguration } from "@/server-actions/saveConfig";
...
const DesignConfigurator = ({configId,imageUrl,imageDimensions}:DesignConfigProps) => {
    ...
    const saveConfig = async({color,finish,material,model,configId}: SaveConfigArgs)=>{
        try {
            setIsPending(true);
            const {
                left: caseLeft,
            ...
            const respone = await saveConfiguration({color,finish,material,model,configId});
            ...
            <Rnd
                ...
                onResizeStop={(_, __, ref, ___, { x, y }) => {
                    setRenderedDimension({
                        height: parseInt(ref.style.height.slice(0, -2)),  // remove 'px' from the end
